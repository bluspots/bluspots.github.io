...
// fixed string. IMPORTANT: material costs are drawn FROM the task's
// already-agreed price (see getCompletionDetails' cap below), never added
// on top of it — a receipt must never show a higher total than what the
// customer actually approved at booking time.
...
function getCompletionDetails(category,taskPrice){
  ...
  return {
    ...
    materials, // materials cost always drawn FROM taskPrice, never added to it
    laborAmount:(taskPrice||0)-materialsCost,
...
export default function App(){
  ...
  // (polling moved below after jobs state + TERMINAL_STATUSES)
  ...
  // Live jobs ref to avoid effect thrash on polling
  ...
  // Profile
  const DEFAULT_PROFILE={name:"Jane Doe",bio:"",photo:null,accountCreatedAt:Date.now(),email:"jane.doe@email.com",phone:"(415) 555-0142"};
  const [profile,setProfile] = usePersistedState("haven_profile",DEFAULT_PROFILE,{
    ...
    validate:v=>{
      ...
      return {
        ...
        email:typeof v.email==="string"&&v.email?v.email:DEFAULT_PROFILE.email,
        phone:typeof v.phone==="string"&&v.phone?v.phone:DEFAULT_PROFILE.phone,
      };
    },
  });
  ...
  // Search-by-symptom now runs through interpretHomeIntent() (module-level
  // — see matchRepairIntent/INTENT_LIBRARY above), the same clean boundary
  // a future AI classifier would implement. Category browsing via the
  // chips below is completely unaffected — it never touches intent
  // matching at all, only a live query does.
  ...
  // The UI's one and only entry point into diagnosis — interpretHomeIntent
  // is the full contract (category/service confidence, urgency,
  // property-aware scope, clarification). tier/matches/clarification are
  // derived from it here rather than rewriting the render logic below,
  // since that logic is already proven correct; what changed is that
  // nothing in this component calls matchRepairIntent directly anymore.
  const homeIntent = q ? interpretHomeIntent(q,{property:selectedAddress}) : null;
  const intentResult = homeIntent ? {
    tier: homeIntent.serviceId ? (homeIntent.serviceConfidence>=70?"high":"medium") : "low",
    matches: homeIntent._matches,
    clarification: homeIntent.clarificationRequired ? {question:homeIntent.clarificationQuestion,options:homeIntent.clarificationOptions} : null,
  } : {tier:"none",matches:[],clarification:null};
  ...
  useEffect(()=>{
    ...
    const tick=async()=>{
      ...
      const activeLinked = currentJobs.filter(j=>j.backendJobId && !isTerminalStatus(j.status));
  ...
  const postJob=()=>{
    ...
    (async()=>{
      ...
      const fixed_customer_labor_price_cents = Math.max(0, Math.floor(customerPriceDollars*100));
      const fixed_pro_labor_payout_cents     = Math.floor(fixed_customer_labor_price_cents * 80 / 100);
  ...
  // True vector PDF via jsPDF (loaded from CDN, window.jspdf.jsPDF — same
  // loading convention as React/Babel). This is entirely separate from the
  ...
  const generateReceiptPDF=(job)=>{
    ...
    const laborLine=laborTotal-materialsCost; // materials are drawn FROM the agreed price, never added on top
    const grandTotal=laborTotal+(job.surge||0)+(job.emergencyFee||0)+(job.tipAmount>0?job.tipAmount:0);
    ...
    const INK=[28,43,58], MUTED=[95,110,122], FAINT=[154,170,182], DIVIDER=[222,217,207], GREEN=[15,120,80], GREEN_BG=[228,246,238];
    const setInk=()=>doc.setTextColor(INK[0],INK[1],INK[2]);
    const setMuted=()=>doc.setTextColor(MUTED[0],MUTED[1],MUTED[2]);
    const setFaint=()=>doc.setTextColor(FAINT[0],FAINT[1],FAINT[2]);
    const divider=(yy,weight=0.75)=>{ doc.setDrawColor(DIVIDER[0],DIVIDER[1],DIVIDER[2]); doc.setLineWidth(weight); doc.line(MARGIN,yy,PAGE_W-MARGIN,yy); };
    const label=(text,x,yy)=>{ doc.setFont("helvetica","bold"); doc.setFontSize(7.5); setFaint(); doc.text(text.toUpperCase(),x,yy,{charSpace:0.6}); };

    const ensureSpace=(needed)=>{
      if(y+needed>PAGE_H-MARGIN-40){ // leave room for the footer band
    ...
    doc.text("HAVEN",MARGIN,y+16);
    doc.setFont("helvetica","normal"); doc.setFontSize(9); setMuted();
    doc.text("Home Services, Made Simple.",MARGIN,y+30);
    ...
    // === SERVICE INFORMATION (four columns) ===
    const svcColW=CONTENT_W/4;
    const svcCols=[
      ["Service", jt?jt.n:(job.custom?.title||"Custom job")],
      ["Category", category||"—"],
      ["Property", job.addressLabel||"Home"],
      ["Date of Service", dateStr],
    ];
    svcCols.forEach(([l,v],i)=>{
      const x=MARGIN+i*svcColW;
      label(l,x,y);
    });
    y+=15;
    doc.setFont("helvetica","bold"); doc.setFontSize(9.5); setInk();
    svcCols.forEach(([l,v],i)=>{
      const x=MARGIN+i*svcColW;
      const lines=doc.splitTextToSize(v,svcColW-10);
      lines.slice(0,3).forEach((line,li)=>doc.text(line,x,y+li*11));
    });
    y+=32; divider(y); y+=24;

    // === PAYMENT TABLE (the visual focus) ===
    ensureSpace(120);
    const qtyColX=MARGIN+CONTENT_W-110, amtColX=MARGIN+CONTENT_W;
    doc.setFont("helvetica","bold"); doc.setFontSize(9); setInk();
    doc.text("PAYMENT",MARGIN,y);
    doc.setFontSize(7.5); setFaint();
    ...
    rows.forEach(([kicker,desc,qty,amt])=>{
      ...
      setMuted(); doc.text(String(qty),qtyColX,y,{align:"right"});
      setInk(); doc.text(`$${amt.toFixed(2)}`,amtColX,y,{align:"right"});
      y+=18;
    });
    ...
    if(workPerformed.length){
      ensureSpace(24+workPerformed.length*14);
      label("Work Performed",MARGIN,y); y+=16;
      doc.setFont("helvetica","normal"); doc.setFontSize(9.5); setInk();
      workPerformed.forEach(item=>{
        ensureSpace(16);
        doc.text("\u2022",MARGIN,y);
        const lines=doc.splitTextToSize(item,CONTENT_W-16);
        lines.forEach((line,li)=>doc.text(line,MARGIN+12,y+li*12.5));
    ...
    if(job.proNotes){
      ensureSpace(40);
      label("Notes from Professional",MARGIN,y); y+=16;
      doc.setFont("helvetica","normal"); doc.setFontSize(9.5); setInk();
      const noteLines=doc.splitTextToSize(job.proNotes,CONTENT_W);
    ...
    // === FOOTER (two columns, always at the bottom of the LAST page) ===
  ...
  const receiptScreen=()=>{
    ...
    const downloadReceipt=()=>{
      ...
      // Fallback — PDF generation unavailable for some reason (e.g. the
      // CDN library failed to load): export still works, just as text.
      try{

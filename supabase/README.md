...
-
- CHUNK 3 adds:
  - `migrations/0004_chunk3_accept_and_decline.sql` — adds `materials_declined` status, `convenience_fee_cents`, and prototype UPDATE RLS for claim + terminals
...
Applying migrations
- Recommended order for greenfield projects: `0001_init.sql`, `0002_chunk2_rls.sql`, `0003_chunk2_grants.sql`, then `0004_chunk3_accept_and_decline.sql` and `0005_chunk3_grants.sql`.
- If prior chunks were already applied, run `0004` then `0005` to add CHUNK 3 without editing migration history.

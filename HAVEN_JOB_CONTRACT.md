...
| `platformFee` | number | ❌ not present | ❌ not present | Not yet modeled on either side. Real backend requirement (see previous row). Any platform/labor margin percentage is founder‑TBD; do not assume a value in code or docs. |
...
- Platform/labor margin percentage is founder‑TBD. This PR does not state
  or imply a percentage anywhere.

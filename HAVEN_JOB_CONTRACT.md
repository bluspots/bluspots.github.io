...
| `fixedCustomerLaborPrice` | number | ✅ resolved via `vjTask.p` / `custom.price` | ⚠️ `payout` (this is the *pro's* payout, not the customer's price — see next row) | For now, service price equals labor, and Haven takes $0 from labor. The pro’s payout equals the labor price. Founder may revisit labor later. |
| `fixedProLaborPayout` | number | ❌ not modeled (Customer App has no concept of what the pro earns) | ✅ `payout` | For now, `fixedProLaborPayout === fixedCustomerLaborPrice` for labor (Haven takes $0 from labor). Founder may revisit this later. |
| `platformFee` | number | ❌ not present | ❌ not present | Not yet modeled on either side. For labor, the platform fee is $0 for now (founder‑locked). Founder may revisit labor in the future. Do not introduce a labor margin or assume a percentage in code or docs. |
...
- Labor margin is locked at $0 to Haven for now. The pro keeps 100% of the
  labor price. The founder may revisit labor later.
